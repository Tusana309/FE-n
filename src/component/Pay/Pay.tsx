          ...
          <div className='thanhtoan'>
            ...
            <div className='ttck'>
              <div className='nganhang'>
                <h4>Ngân hàng {profile.bank_name}</h4>
              </div>
              <div className='taikhoan '>
                <div className='trai flex flex-col gap-2'>
                  <p>Số tài khoản</p>
                  <p>Chủ tài khoản</p>
                ...
                <div className='phai flex flex-col gap-2'>
                  <p>{state?.result[0]?.user.account_number}</p>
                  <p>{state?.result[0]?.user.account_name}</p>
                  <p>{state?.price} VND</p>
                  <p>{state?.code}</p>
                  {/* <img src={profile.qr_img} alt='' /> */}
                  <div className='w-[400px]'>
                    <img className='w-full h-full object-cover' src={state?.result[0]?.user.qr_img} alt='' />
                  </div>
                </div>
              </div>
            </div>
            <div className='xn'>
              <button onClick={() => navigate('/')}>Tôi đã hoàn tất thanh toán</button>
            </div>
          </div>
        </div>
      </div>
