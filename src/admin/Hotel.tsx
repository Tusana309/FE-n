...
import { getBooking, getHotelPartner, updateStatus } from 'src/apis/booking.api'
...
import { HotelBody, createHotel, updateHotel, getHotelId } from 'src/apis/hotel.api'
const customStyles = {
  content: {
    top: '50%',
    left: '50%',
...
const ModalUpdate = ({ isOpen, setIsOpen, body, setBody, result }: Props) => {
  ...
  const mutation = useMutation((body: HotelBody) => createHotel(body))
  const mutationUpdate = useMutation((body: HotelBody) => updateHotel(body, profile.hotel_id))
  const { data: hotelInfo } = useQuery({
    queryKey: ['', profile.hotel_id],
    queryFn: () => getHotelId(profile.hotel_id)
  })
  ...
  return (
    ...
    >
      ...
      <div className='w-[600px] mt-4'>
        <form action='' className='flex flex-col gap-2' onSubmit={profile.hotel_id ? handleUpdateSubmit : handleSubmit}>
          <input
            name='hotel_name'
            type='text'
            // value={body.departure}
            placeholder={hotelInfo?.data?.result !== null ? hotelInfo?.data?.result[0]?.hotel_name : 'Tên dịch vụ'}
          ...
          <input
            ...
            placeholder={hotelInfo?.data?.result !== null ? hotelInfo?.data?.result[0]?.location : 'Địa điểm'}
            className='py-2 px-3 rounded-md'
            onChange={handleOnChange}
          />
          <input
            ...
            placeholder={hotelInfo?.data?.result !== null ? hotelInfo?.data?.result[0]?.price : 'Giá tiền'}
          ...
          <input
            ...
            placeholder={
              hotelInfo?.data?.result !== null ? hotelInfo?.data?.result[0]?.total_of_room : 'Số lượng phòng'
            }
            className='py-2 px-3 rounded-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'
            onChange={handleOnChange}
          />
          <input
            ...
            type='text'
            // value={body.destination_time}
            placeholder={hotelInfo?.data?.result !== null ? hotelInfo?.data?.result[0]?.description :'Mô tả dịch vụ'}
            className='py-2 px-3 rounded-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'
            onChange={handleOnChange}
          />
          <input
            ...
            type='text'
            // value={body.price}
            placeholder={hotelInfo?.data?.result !== null ? hotelInfo?.data?.result[0]?.hotel_facilities :'Các dịch vụ khách sạn'}
            className='py-2 px-3 rounded-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'
            onChange={handleOnChange}
          />
          <input
          ...
          <div className='flex justify-center'>
            <img style={{ maxWidth: '315px' }} src={hotelInfo?.data?.result[0]?.hotel_img} alt=''></img>
          </div>
          <div className='flex justify-center'>
            <button
              type='submit'
  ...
  return (
    <div className='p-4 col-span-10 bg-[#eee]'>
      ...
      <div className='flex justify-between mx-10 pb-6 border-b-2'>
        <h1 className='text-xl font-bold'>Vé khách sạn</h1>
