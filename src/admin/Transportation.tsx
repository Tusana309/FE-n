...
import { TransBody, createTrans, getTrans, updateTrans } from 'src/apis/transportation.api'
...
const ModalUpdate = ({ isOpen, setIsOpen, body, setBody, result }: Props) => {
  ...
  const { data: transInfo } = useQuery({
    queryKey: ['', profile.transportation_id],
    queryFn: () => getTrans(profile.transportation_id)
  })
      ...
      <div className='w-[600px] mt-4'>
        ...
        >
          <input
            ...
            placeholder={transInfo?.data?.result !== null ? transInfo?.data?.result[0]?.tranportation_name : 'Tên dịch vụ'}
          ...
          <input
            ...
            placeholder={transInfo?.data?.result !== null ? transInfo?.data?.result[0]?.location : 'Địa điểm'}
            className='py-2 px-3 rounded-md'
            onChange={handleOnChange}
          />
          <input
            ...
            placeholder={transInfo?.data?.result !== null ? transInfo?.data?.result[0]?.price : 'Giá tiền'}
          ...
          <input
            ...
            placeholder={transInfo?.data?.result !== null ? transInfo?.data?.result[0]?.description :'Mô tả dịch vụ'}
            className='py-2 px-3 rounded-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'
            onChange={handleOnChange}
          />
          <input
            ...
            // value={body.destination_time}
            placeholder={
              transInfo?.data?.result !== null ? transInfo?.data?.result[0]?.total_of_vehicle : 'Số lượng xe vận chuyển'
            }
            className='py-2 px-3 rounded-md [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none'
            onChange={handleOnChange}
          />
          ...
          <div className='flex justify-center'>
            <img style={{ maxWidth: '315px' }} src={transInfo?.data?.result[0]?.car_vehicle} alt=''></img>
          </div>
          <div className='flex justify-center'>
            <button
              type='submit'
